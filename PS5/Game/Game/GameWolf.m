...
-(void)wolfDie{
    ...
    UIImage* wolfsImage = [UIImage imageNamed:@"wolfdie.png"];
    CGImageRef imageRef;
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            cropRect = CGRectMake(0+j*237, 0+i*185, 237,185);
    ...
    [self performSelector:@selector(restoreDeadView) withObject:Nil afterDelay:animationView.animationDuration];
